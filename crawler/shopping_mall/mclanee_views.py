...
def mclanee_product_list_provider(main_url, page_list):
    ...
    for i in range(len(page_list)):
        is_best = 0
        if page_list[i].startswith('http://www.mclanee.co.kr/product/list.html?cate_no=101'):
            is_best = 1
...
# def mclanee_update_database(product_list):
#     queryset = Product.objects.filter(shopping_mall=10)
#     if queryset.count() == 0:
#         pass
#     else:
#         origin_list = []
...
def mclanee_update_product_list(all_info_list):
    remove_list = []
...
def mclanee_update_database(all_info_list):
    queryset = BagImage.objects.filter(product__shopping_mall=10)
    if queryset.count() == 0:
        pass
    else:
        origin_list = []
...
def mclanee_make_model_table(all_info_list):
    for i in range(len(all_info_list)):
        p, _ = Product.objects.update_or_create(shopping_mall=10, product_name=all_info_list[i][8],
        ...
        for j in range(len(all_info_list[i][3])):
            ...
            for k in range(len(colortab_list)):
                ...
                if any(c in colortab_list[k] for c in ('베이지', '타프베이지', '코코아', '오트밀')):
                ...
                if any(c in colortab_list[k] for c in ('애쉬플럼', '플럼', '보라', '퍼플', '보르도', '보로도', '바이올렛', '라벤더')):
